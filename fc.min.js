...
window.fc=function(){function g(a){c[a.name].forEach(function(b){void 0!==b.thisArg?b.callback.call(b.thisArg,a):b.callback(a)});return a}function d(a){if("string"!=typeof a)throw new TypeError("fc.publish: `message` must be a string.");if(!/^[a-z0-9_/ /:/-]+$/i.test(a))throw Error("fc.publish: `message` must be a string containing at least one character, and composed only of alphanumeric characters, underscores, spaces, colons, and dashes.");if(3<a.split(":").length)throw Error("fc.publish: `message` may contain at most three colons, example: `namespace:event:identifier`.");
